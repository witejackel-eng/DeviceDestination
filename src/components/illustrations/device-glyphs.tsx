  ...
  return (
    <svg viewBox="0 0 64 64" className={className} role="img" aria-label="Dome camera">
      ...
      <path
        ...
        fill={accent ? "var(--accent-soft)" : "var(--canvas-alt)"}
        stroke={STROKE}
        strokeWidth="2"
      />
      <circle cx="32" cy="13" r="7.5" fill="var(--background)" stroke={STROKE} strokeWidth="2" />
      <circle cx="32" cy="13" r="3.4" fill={accent ? "var(--accent)" : STROKE} />
...
export function BulletCameraGlyph({ className, accent = true }: GlyphProps) {
  return (
    <svg viewBox="0 0 64 64" className={className} role="img" aria-label="Bullet camera">
      ...
      <rect x="18" y="22" width="30" height="18" rx="6" fill={accent ? "var(--accent-soft)" : "var(--canvas-alt)"} stroke={STROKE} strokeWidth="2" />
      <circle cx="48" cy="31" r="9" fill="var(--background)" stroke={STROKE} strokeWidth="2" />
      <circle cx="48" cy="31" r="4" fill={accent ? "var(--accent)" : STROKE} />
...
export function NvrGlyph({ className, accent = true }: GlyphProps) {
  return (
    <svg viewBox="0 0 64 64" className={className} role="img" aria-label="NVR storage">
      ...
      <rect x="13" y="24" width="20" height="5" rx="1.5" fill="var(--background)" stroke={STROKE} strokeWidth="1.4" />
      <rect x="13" y="32" width="20" height="5" rx="1.5" fill="var(--background)" stroke={STROKE} strokeWidth="1.4" />
      <rect x="13" y="40" width="20" height="4" rx="1.5" fill="var(--background)" stroke={STROKE} strokeWidth="1.4" />
      <circle cx="47" cy="26" r="2.4" fill={accent ? "var(--accent)" : STROKE} />
...
export function BiometricGlyph({ className, accent = true }: GlyphProps) {
  return (
    <svg viewBox="0 0 64 64" className={className} role="img" aria-label="Biometric device">
      ...
      <rect x="22" y="17" width="20" height="20" rx="3" fill="var(--background)" stroke={STROKE} strokeWidth="1.6" />
      <path
        d="M32,20 a6,6 0 0 1 6,6 M32,20 a6,6 0 0 0 -6,6 M28,30 a4,4 0 0 1 8,0"
        fill="none"
        stroke={accent ? "var(--accent)" : STROKE}
        strokeWidth="1.6"
        strokeLinecap="round"
      />
      <circle cx="32" cy="45" r="3" fill={accent ? "var(--accent-soft)" : "var(--background)"} stroke={STROKE} strokeWidth="1.6" />
    </svg>
  );
}
    ...
    <svg viewBox="0 0 64 64" className={className} role="img" aria-label="PoE switch">
      ...
      {[0, 1, 2, 3, 4, 5].map((index) => (
        <rect
          ...
          width="5"
          height="8"
          fill={accent && index === 2 ? "var(--accent)" : "var(--background)"}
          stroke={STROKE}
          strokeWidth="1.3"
        />
      ))}
