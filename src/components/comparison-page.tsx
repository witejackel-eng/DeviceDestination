  ...
  return (
    <div className="container-standard section-space !pt-14" style={{ background: "var(--surface)" }}>
      ...
      <div className="mt-3 flex flex-wrap items-end justify-between gap-5 max-w-2xl">
        <h1 className="section-title">Compare exact models.</h1>
        {selected.length > 0 && (
      ...
      <div className="mt-8 flex flex-wrap items-center gap-3 rounded-[var(--radius-container)] border border-[var(--border)] p-4 bg-[var(--surface-subtle)]">
        ...
        <select
          ...
          className="h-11 min-w-0 flex-1 rounded-[var(--radius-btn)] border border-[var(--border)] bg-[var(--surface)] px-3 sm:max-w-md"
        >
          <option value="">
        ...
        <span className="text-xs text-[var(--text-muted)]">{selected.length}/4 selected</span>
      </div>
      {!selected.length ? (
        <div className="surface-card mt-10 grid min-h-[360px] place-content-center p-8 text-center" style={{ background: "var(--surface)" }}>
          <Plus className="mx-auto text-[var(--accent)]" size={32} />
        ...
        <div className="mt-8 overflow-x-auto rounded-[var(--radius-container)] border border-[var(--border)]" style={{ background: "var(--surface)" }}>
          <table className="w-full min-w-[760px] border-collapse text-left">
            <thead>
              <tr>
                <th className="w-[210px] bg-[var(--surface-subtle)] p-4 text-sm font-bold sticky left-0 z-10">Product</th>
                {selected.map((product) => (
                  <th
                    key={product.id}
                    className="relative min-w-[240px] border-l border-[var(--border)] p-5 align-top bg-[var(--surface)]"
                  >
                    <button
                      type="button"
                      onClick={() => remove(product.id)}
                      className="absolute right-3 top-3 grid h-9 w-9 place-items-center rounded-[var(--radius-btn)] border border-[var(--border)] bg-[var(--surface)]"
                      aria-label={`Remove ${product.model}`}
                    >
                      <X size={15} />
                    </button>
                    <div className="relative mb-4 aspect-[1.3] rounded-[var(--radius-card)] bg-[var(--surface-subtle)]">
                      <Image
            ...
            <tbody>
              <tr className="border-t border-[var(--border)]">
                <th className="bg-[var(--surface-subtle)] p-4 text-sm font-bold sticky left-0 z-10">Availability</th>
                {selected.map((product) => (
                  <td key={product.id} className="border-l border-[var(--border)] p-5 text-sm">
                    <span className={`inline-flex items-center gap-1.5 font-semibold ${product.stockStatus === "in_stock" ? "text-[var(--success)]" : "text-[var(--text-muted)]"}`}>
              ...
              <tr className="border-t border-[var(--border)]">
                <th className="bg-[var(--surface-subtle)] p-4 text-sm font-bold sticky left-0 z-10">Warranty</th>
                {selected.map((product) => (
                  <td key={product.id} className="border-l border-[var(--border)] p-5 text-sm font-semibold">
              ...
              {specLabels.map((label) => (
                <tr key={label} className="border-t border-[var(--border)]">
                  <th className="bg-[var(--surface-subtle)] p-4 text-sm font-bold sticky left-0 z-10">{label}</th>
                  {selected.map((product) => {
                    ...
                    return (
                      <td
                        key={product.id}
                        className={`border-l border-[var(--border)] p-5 text-sm leading-6 ${hasValue ? "text-[var(--text-secondary)] font-medium" : "text-[var(--text-muted)]"}`}
                      >
                        {value ?? "—"}
                      </td>
                    );
